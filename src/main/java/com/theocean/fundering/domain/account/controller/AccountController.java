...
public class AccountController {
    ...
    // (기능) 펀딩 출금가능 금액 조회
    @Operation(summary = "펀딩 출금가능 금액 조회", description = "postId에 해당하는 게시글의 Account의 출금가능 금액을 조회한다.", responses = {
            @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = BalanceResponse.class)))
    })
